...
FROM review r
FORCE INDEX (idx_review_ticket)
JOIN ticket t FORCE INDEX (idx_ticket_event) ON r.ticket_ID = t.ticket_ID
JOIN events e ON t.event_ID = e.event_ID
JOIN performances p FORCE INDEX (idx_perf_event_artist) 
...
SELECT 
    ...
    ROUND(AVG(r.overall_impression), 2) AS avg_overall_impression
FROM review r
IGNORE INDEX (idx_review_ticket)
JOIN ticket t IGNORE INDEX (idx_ticket_event) ON r.ticket_ID = t.ticket_ID
JOIN events e ON t.event_ID = e.event_ID
JOIN performances p IGNORE INDEX (idx_perf_event_artist) 
    ON e.event_ID = p.event_ID AND t.event_ID = p.event_ID
