SELECT
    ...
    a.artist_name,
    SUM(
        CAST(r.artist_performance AS UNSIGNED) + CAST(r.sound_and_lighting AS UNSIGNED) + CAST(r.stage_presence AS UNSIGNED) + CAST(r.event_organization AS UNSIGNED) + CAST(r.overall_impression AS UNSIGNED)
    ) AS total_score
FROM
    review r
